...
public class WorkflowController : ControllerBase
{
    ...
    public async Task<IActionResult> ExecuteCommand(Guid processId, string command, string identityId)
    {
        // Get available commands for the process
        var commands = await WorkflowInit.Runtime.GetAvailableCommandsAsync(processId, identityId);
       // Console.WriteLine($"All Available Commands for {identityId}: {string.Join(", ", commands.Select(c => c.CommandName))}");

        // Check if there are any commands available
        if (commands == null || !commands.Any())
        {
            return BadRequest($"No available commands for process {processId} and identity {identityId}");
        }
       // Console.WriteLine($"All Available Commands for {identityId}: {string.Join(", ", commands.Select(c => c.CommandName))}");

        // Find the specified command
        var workflowCommand = commands.FirstOrDefault(c => c.CommandName == command);
        if (workflowCommand == null)
        {
            return BadRequest($"Command {command} not found for process {processId}");
        }
